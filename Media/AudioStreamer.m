...
//inClientData是在AudioSessionInitialize函数中指定的最后一个参数的数据  第二个参数表示中断状态,有开始中断和结束中断两种
void MyAudioSessionInterruptionListener(void *inClientData, UInt32 inInterruptionState)
{
	AudioStreamer* streamer = (AudioStreamer *)inClientData;
...
- (void)startInternal
{
	...
	@synchronized(self)
	{
		...
		UInt32 sessionCategory = kAudioSessionCategory_MediaPlayback;  //设置音频种类用于播放录制声音或其他声音 一般不混合ipod里面的声音
		AudioSessionSetProperty (//第一个参数表示要设置值的标志符号,当前是属于音频种类标志
...
- (void)start
{
	@synchronized (self)
	{
		...
		else if (state == AS_INITIALIZED)  //默认state为AS_INITIALIZED 表示还没初始化
		{
			NSAssert([[NSThread currentThread] isEqual:[NSThread mainThread]],  // 前面是条件,后面是打印的字符,如不满足,就打印,并抛出错误
				@"Playback can only be started from the main thread.");
			notificationCenter =
				[[NSNotificationCenter defaultCenter] retain];
			self.state = AS_STARTING_FILE_THREAD; //表示开始文件线程
			internalThread =
				[[NSThread alloc]
					initWithTarget:self
	...
	eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		if (!httpHeaders)
		{
			...
			CFRelease(message);
			NSLog(@"message:%@ AndHeaders %@", message, httpHeaders);
			...
			(state == AS_STOPPED && stopReason == AS_STOPPING_TEMPORARILY))
		{
			...
			if (state == AS_FLUSHING_EOF || buffersUsed == kNumAQBufs - 1)
			{
				...
				else
				{
                    ...
                    {
                        NSLog(@"AudioStreamer2:%@ andURL:%@",self,url);    
                    }
...
- (void)handleInterruptionChangeToState:(AudioQueuePropertyID)inInterruptionState
{
	...
	{
		AudioSessionSetActive( true );  //true 音频队列继续运行,fause 停止运行
		[self pause];
	}
}
#endif
